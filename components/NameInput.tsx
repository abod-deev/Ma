...
export const NameInput: React.FC<NameInputProps> = ({ onStart, isRtl, initialText = '' }) => {
  ...
  return (
    <div className={`max-w-md mx-auto mt-2 md:mt-8 px-0 md:px-6 animate-in slide-in-from-bottom-10 duration-700 ${isRtl ? 'text-right' : 'text-left'}`}>
      <div className="glass-panel border-slate-200 dark:border-white/10 rounded-xl md:rounded-2xl p-4 md:p-6 shadow-2xl overflow-hidden relative transition-colors duration-500">
        ...
        <div className="flex items-center justify-between mb-3 md:mb-6">
          <div className="flex items-center gap-2 md:gap-3">
            <div className="bg-blue-100 dark:bg-blue-600/10 p-1.5 md:p-2 rounded-lg border border-blue-500/20">
              <UserPlus className="w-4 h-4 md:w-5 md:h-5 text-blue-600 dark:text-blue-400" />
            </div>
          ...
          <div className="flex items-center gap-1.5 px-2 py-0.5 md:px-3 md:py-1 bg-blue-100 dark:bg-blue-600/10 border border-blue-500/20 rounded-full text-blue-600 dark:text-blue-400 text-[9px] md:text-[10px] font-black uppercase tracking-widest">
            <Users className="w-3 h-3" />
            <span>{count}</span>
          </div>
        </div>
        
        <p className="text-slate-500 dark:text-slate-400 mb-2 md:mb-4 text-[10px] md:text-[11px] font-medium leading-relaxed opacity-80">
          {isRtl 
        ...
        <textarea
          ...
          className={`w-full h-32 md:h-56 bg-white/50 dark:bg-black/40 border border-slate-300 dark:border-white/5 rounded-xl p-3 md:p-4 text-xs md:text-sm text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-700 focus:outline-none focus:ring-2 focus:ring-blue-500/30 transition-all ${isRtl ? 'text-right' : 'text-left'} font-medium resize-none`}
        />
